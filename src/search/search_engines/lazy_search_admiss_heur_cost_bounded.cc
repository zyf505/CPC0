#include "lazy_search_admiss_heur_cost_bounded.h"
...
namespace lazy_search_adimss_heur_cost_bounded {
static const int DEFAULT_LAZY_BOOST = 1000;

LazySearchAdimssHerCostBounded::LazySearchAdimssHerCostBounded(const Options &opts)
    : SearchEngine(opts),
...
void LazySearchAdimssHerCostBounded::set_pref_operator_heuristics(
    vector<Heuristic *> &heur) {
    preferred_operator_heuristics = heur;
}

void LazySearchAdimssHerCostBounded::initialize() {
...
void LazySearchAdimssHerCostBounded::get_successor_operators(vector<const GlobalOperator *> &ops) {
    assert(ops.empty());
...
void LazySearchAdimssHerCostBounded::generate_successors() {
    ...
    int new_h=0;
    for (Heuristic *heuristic : heuristics){
      new_h=max(new_h,current_eval_context.get_heuristic_value(heuristic));
    }
    if((current_real_g + new_h)>bound){
      //cout<<"current_real_g="<<current_real_g<<",h:"<<new_h<<",total:"<<current_real_g+new_h<<",so skipping adding node to open list"<<endl;
      return;
    }
...
//	else{
//	  cout<<"actual new_real_g="<<new_real_g<<" too big,so skipping adding node to open list"<<endl;
//	}
    }
}

SearchStatus LazySearchAdimssHerCostBounded::fetch_next_state() {
    if (open_list->empty()) {
...
SearchStatus LazySearchAdimssHerCostBounded::step() {
    // Invariants:
...
void LazySearchAdimssHerCostBounded::reward_progress() {
    open_list->boost_preferred();
}

void LazySearchAdimssHerCostBounded::print_checkpoint_line(int g) const {
    cout << "[g=" << g << ", ";
    statistics.print_basic_statistics();
    cout << "]" << endl;
}

void LazySearchAdimssHerCostBounded::print_statistics() const {
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    LazySearchAdimssHerCostBounded *engine = nullptr;
    if (!parser.dry_run()) {
        engine = new LazySearchAdimssHerCostBounded(opts);
        /*
...
static SearchEngine *_parse_greedy(OptionParser &parser) {
    ...
    LazySearchAdimssHerCostBounded *engine = 0;
    if (!parser.dry_run()) {
...
static Plugin<SearchEngine> _plugin("lazy_cbo", _parse);
static Plugin<SearchEngine> _plugin_greedy("lazy_greedy_admiss_cost_bounded", _parse_greedy);
